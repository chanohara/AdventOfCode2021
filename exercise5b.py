...
def markDiagonalLineOnBoard(board, line):
    temp = line.start.copy()
    ...
    while True:
        ...
        if temp[0] == line.end[0] and temp[1] == line.end[1]:
            break
        
        if temp[0] > line.end[0]:
            temp[0] -=1
        else: 
            temp[0] += 1

        if temp[1] > line.end[1]:
            temp[1] -=1
        else: 
            temp[1] += 1
...
def determineOverlaps( input ):
    ...
    for line in lines:
        if line.isVerticalOrHorizontal( ) != True:
            markDiagonalLineOnBoard(board, line)
        else:
